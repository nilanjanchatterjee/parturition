...
The results are provided as a set of plots per track (`parturition_analysis_[window]h_threshold_[threshold].pdf`) and a csv summarizing parturition events (`parturition_output_[window]h_threshold_[threshold].csv`). In addition, movement metrics and parturition indicators are added to the move2 output, indicating whether the locations satisfy the clustering criteria or not.
